...
class MaceModel(ModelInterface):
    ...
    def __init__(
            ...
            *,
            ...
            compute_atomic_stresses: bool = True,
    ...
    def forward(  # noqa: C901
        ...
        if energies is not None:
            results["energies"] = energies.detach()
        else:
            results["energies"] = torch.zeros(self.n_systems, device=self.device)
        ...
        if self.compute_atomic_stresses:
            virials = out["atomic_virials"]
            if virials is not None:
                results["virials"] = virials.detach()

        if self.include_additional_properties:
        ...
        if self.compute_correction:
            energy_correction, forces_correction, virial_correction = self.dftd3_corrections.compute_corrections(state_positions=state.positions, state_cell=state.cell,
                                                       system_idx=state.system_idx, unit_shifts=unit_shifts,
