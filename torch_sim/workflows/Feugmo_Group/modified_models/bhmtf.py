...
def calculate_external_force(energies, momenta, masses, virials, Fe_vec):
    "F_ext or driving force for HNEMDEC"
    n_atoms, m_tot = masses[0], torch.sum(masses)
...
def per_atom_virial(dr_vec ,potential_details, positions, mapping, device, dtype, compute_forces, compute_coulombic_term):
    dr_vec.requires_grad_(True)
    pair_mask, ionic_charge_i, ionic_charge_j, sigma_ij, a,b,c,d, cutoff = potential_details
    ...
    potential_energy = (t_1 + t_2 - t_3 - t_4) if compute_forces else (t_2 - t_3 - t_4)
    potential_energy = torch.where(dr>0, potential_energy, torch.zeros_like(potential_energy))
    ...
    pair_energies = (potential_energy * mask.float())  # Faster than torch.where
    # deleting non-required variable:
    del t_1, t_2, t_3, t_4,
    if compute_forces:
        pair_forces = fumi_tosi_pair_force(dr, potential_energy, True).detach() # Initialize force tensor
    else:
        pair_forces = None
    del potential_energy, dr
    ...
    with torch.no_grad():
        ...
        atom_virial = torch.zeros((positions.shape[0], 3, 3), dtype=term_tensor.dtype, device=device)
        ...
        # dr_vec.detach()
        return pair_energies, atom_energies, pair_forces, total_virial, atom_virial
...
def fumi_tosi_pair(
    ...
    # Calculate the potential energy
    potential_energy = (t_1 + t_2 - t_3 - t_4) if compute_coulombic_term else t_2 - t_3 - t_4
...
def fumi_tosi_pair_force(
    ...
    force = torch.autograd.grad(
        ...
        retain_graph=True if compute_virial else False,
        create_graph=False,
    )[0]
...
class FumiTosiModel(ModelInterface):
    ...
    Attributes:
        ...
        compute_coulombic_term (bool): Whether to compute coulombic term.
    ...
    def __init__(
            ...
            *, # Force keyword-only arguments
            ...
            compute_coulombic_term: bool = False,
        ...
        Args:
            ...
            compute_stress (bool): Whether to compute stress tensor.
            compute_virials (bool): Whether to compute system & per_atom virials
        ...
        self._compute_coulombic_term = compute_coulombic_term
        self._compute_stress = compute_stress
        self._compute_virials = compute_virials
    ...
    def unbatched_forward(
        ...
        if self.compute_forces or self.compute_stress:
            if not self._compute_virials:
                # Calculate forces
                pair_forces = fumi_tosi_pair_force(
                    distances, pair_energies, self._compute_virials
                )
            # apply cutoff
        ...
        if self._compute_external_force:
            if isinstance(state, ts.integrators.MDState): # only compute when doing Molecular Dynamics
                external_force = self.external_force_fn(atom_energies, state.momenta, state.masses, atom_virial, self.Fe_vec)
    ...
    def forward(self, state: SimState | StateDict) -> dict[str, torch.Tensor]:
        ...
        for key in ("energies", "forces", "stresses", "virials"):
            if key in properties:
